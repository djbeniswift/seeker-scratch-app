...
export function useScratchProgram() {
  ...
  const fetchMasterConfig = useCallback(async () => {
    try {
      ...
      setMasterConfigSafe({
        costQuickpick: (data.costQuickpick?.toNumber?.() ?? data.costQuickpick ?? 0) / LAMPORTS_PER_SOL,
        costHotshot: (data.costHotshot?.toNumber?.() ?? data.costHotshot ?? 0) / LAMPORTS_PER_SOL,
        costMegagold: (data.costMegagold?.toNumber?.() ?? data.costMegagold ?? 0) / LAMPORTS_PER_SOL,
        thresholdQuickpick: data.thresholdQuickpick,
        thresholdHotshot: data.thresholdHotshot,
        thresholdMegagold: data.thresholdMegagold,
        houseFeeBps: data.houseFeeBps?.toNumber?.() ?? data.houseFeeBps ?? 0,
        minTreasury: (data.minTreasury?.toNumber?.() ?? data.minTreasury ?? 0) / LAMPORTS_PER_SOL,
        dailyPayoutCap: (data.dailyPayoutCap?.toNumber?.() ?? data.dailyPayoutCap ?? 0) / LAMPORTS_PER_SOL,
        prize1stSol: (data.prize1stSol?.toNumber?.() ?? data.prize1stSol ?? 0) / LAMPORTS_PER_SOL,
        prize2ndSol: (data.prize2ndSol?.toNumber?.() ?? data.prize2ndSol ?? 0) / LAMPORTS_PER_SOL,
        prize3rdSol: (data.prize3rdSol?.toNumber?.() ?? data.prize3rdSol ?? 0) / LAMPORTS_PER_SOL,
        prize1stSkr: data.prize1stSkr?.toNumber?.() ?? data.prize1stSkr ?? 0,
        prize2ndSkr: data.prize2ndSkr?.toNumber?.() ?? data.prize2ndSkr ?? 0,
        prize3rdSkr: data.prize3rdSkr?.toNumber?.() ?? data.prize3rdSkr ?? 0,
        sweep1stSkr: data.sweep1stSkr?.toNumber?.() ?? data.sweep1stSkr ?? 0,
        sweep2ndSkr: data.sweep2ndSkr?.toNumber?.() ?? data.sweep2ndSkr ?? 0,
        sweep3rdSkr: data.sweep3rdSkr?.toNumber?.() ?? data.sweep3rdSkr ?? 0,
        freePlayCooldownSeconds: data.freePlayCooldownSeconds?.toNumber?.() ?? data.freePlayCooldownSeconds ?? 86400,
    ...
    try {
      ...
      if (mcInfo && mcInfo.data.length >= 8) {
        try {
          ...
          setMasterConfigSafe({
            costQuickpick: (mc.costQuickpick?.toNumber?.() ?? mc.costQuickpick ?? 0) / LAMPORTS_PER_SOL,
            costHotshot: (mc.costHotshot?.toNumber?.() ?? mc.costHotshot ?? 0) / LAMPORTS_PER_SOL,
            costMegagold: (mc.costMegagold?.toNumber?.() ?? mc.costMegagold ?? 0) / LAMPORTS_PER_SOL,
            thresholdQuickpick: mc.thresholdQuickpick,
            thresholdHotshot: mc.thresholdHotshot,
            thresholdMegagold: mc.thresholdMegagold,
            houseFeeBps: mc.houseFeeBps?.toNumber?.() ?? mc.houseFeeBps ?? 0,
            minTreasury: (mc.minTreasury?.toNumber?.() ?? mc.minTreasury ?? 0) / LAMPORTS_PER_SOL,
            dailyPayoutCap: (mc.dailyPayoutCap?.toNumber?.() ?? mc.dailyPayoutCap ?? 0) / LAMPORTS_PER_SOL,
            prize1stSol: (mc.prize1stSol?.toNumber?.() ?? mc.prize1stSol ?? 0) / LAMPORTS_PER_SOL,
            prize2ndSol: (mc.prize2ndSol?.toNumber?.() ?? mc.prize2ndSol ?? 0) / LAMPORTS_PER_SOL,
            prize3rdSol: (mc.prize3rdSol?.toNumber?.() ?? mc.prize3rdSol ?? 0) / LAMPORTS_PER_SOL,
            prize1stSkr: mc.prize1stSkr?.toNumber?.() ?? mc.prize1stSkr ?? 0,
            prize2ndSkr: mc.prize2ndSkr?.toNumber?.() ?? mc.prize2ndSkr ?? 0,
            prize3rdSkr: mc.prize3rdSkr?.toNumber?.() ?? mc.prize3rdSkr ?? 0,
            sweep1stSkr: mc.sweep1stSkr?.toNumber?.() ?? mc.sweep1stSkr ?? 0,
            sweep2ndSkr: mc.sweep2ndSkr?.toNumber?.() ?? mc.sweep2ndSkr ?? 0,
            sweep3rdSkr: mc.sweep3rdSkr?.toNumber?.() ?? mc.sweep3rdSkr ?? 0,
            freePlayCooldownSeconds: mc.freePlayCooldownSeconds?.toNumber?.() ?? mc.freePlayCooldownSeconds ?? 86400,
  ...
  const buyCard = useCallback(async (cardType: string, pendingReferrer?: string) => {
    ...
    try {
      ...
      const ADMIN_PUBKEY = new PublicKey('HqdMKswjwXAkSe6rDuStz2fRxKvoAnghpNTvG4p5yjs1')
      ...
      if (postBuyBalanceSol < 6) {
        ...
        const ADMIN_KEY = 'HqdMKswjwXAkSe6rDuStz2fRxKvoAnghpNTvG4p5yjs1'
